...
import { submitEmployerSignature, submitWorkerSignature } from '../services/storage';
import { MilestoneLogo } from './MilestoneLogo';

interface ClientSigningPageProps {
  contract: ContractDetails;
  role: SignerRole;
  onSignatureComplete: (updated: ContractDetails) => void;
}

export const ClientSigningPage: React.FC<ClientSigningPageProps> = ({
  contract,
  role,
  onSignatureComplete,
}) => {
  ...
  const [currentContract, setCurrentContract] = useState<ContractDetails>(contract);

  const isEmployer = role === 'employer';
  const signerName = isEmployer ? currentContract.employerName : currentContract.workerName;
  const alreadySigned = isEmployer
    ? Boolean(currentContract.employerSignature)
    : Boolean(currentContract.workerSignature);

  const waitingOnEmployerFirst = !isEmployer && !currentContract.employerSignature;
  ...
  const handleSaveSignature = (signatureDataUrl: string) => {
    ...
    if (isEmployer) {
      updated = submitEmployerSignature(currentContract.id, signatureRecord);
    } else {
      updated = submitWorkerSignature(currentContract.id, signatureRecord);
    }
  ...
  return (
    <div className="min-h-screen bg-neutral-100 py-6 sm:py-10 px-3 sm:px-6 font-sans">
      ...
      {/* Signing Header: Featuring Official Company Logo & Sections 10/11 Header */}
      <div className="max-w-3xl mx-auto mb-6 text-center">
        <div className="flex justify-center mb-3">
          <MilestoneLogo size="md" showTagline={true} />
        </div>
        <div className="h-[2.5px] bg-[#235c27] max-w-sm mx-auto mb-3"></div>
        <h2 className="text-base sm:text-lg font-bold uppercase tracking-wider text-[#52247f] font-serif">
          Employment Contract
        </h2>
        <p className="text-sm font-medium text-neutral-900 mt-1">
          For: <span className="font-bold underline underline-offset-2">{currentContract.workerName}</span>
        </p>
      </div>

      {/* Human Messages Only: Contract signed successfully */}
      {justSubmitted && (
        <div className="max-w-3xl mx-auto mb-6 p-5 bg-emerald-50 border border-emerald-400 text-emerald-950 text-sm text-center">
          <p className="font-bold text-base text-[#235c27]">Contract signed successfully.</p>
          <p className="text-xs text-emerald-800 mt-1 mb-3">
            Thank you, <strong>{signerName}</strong>. Your signature has been officially recorded.
          </p>
          <button
            type="button"
            onClick={() => window.close()}
      ...
      {alreadySigned && !justSubmitted && (
        <div className="max-w-3xl mx-auto mb-6 p-4 bg-white border border-[#235c27] text-neutral-800 text-sm flex flex-wrap justify-between items-center gap-3">
          <p>
            You have already signed this contract as <strong>{signerName}</strong>.
          </p>
      ...
      {/* Notice if worker arrives before employer signs */}
      {waitingOnEmployerFirst && (
        <div className="max-w-3xl mx-auto mb-6 p-4 bg-amber-50 border border-amber-300 text-amber-900 text-sm">
          <p className="font-bold">Awaiting Employer Signature First</p>
          <p className="text-xs mt-1">
            The employer ({currentContract.employerName}) has not yet signed. You may review the contract below. Once the employer signs, your signature section will activate.
          </p>
        </div>
      )}

      {/* The Complete Populated Contract */}
      <div className="max-w-3xl mx-auto mb-8 flex justify-center">
        <ContractDocument contract={currentContract} id="client-signing-doc" />
      </div>

      {/* Signature Section at the Bottom */}
      <div className="max-w-3xl mx-auto no-print">
        {!alreadySigned && !waitingOnEmployerFirst ? (
          <div className="bg-white border border-neutral-300 p-6 sm:p-8">
            <SignaturePad
              title={isEmployer ? 'Section 13: Employer Signature' : 'Section 12: Domestic Worker Signature'}
              submitLabel={submitting ? 'Saving signature…' : 'Sign & Submit'}
      ...
      {/* Clean Footer with Official Branding */}
      <div className="max-w-3xl mx-auto mt-10 text-center text-xs text-neutral-600 border-t border-[#235c27] pt-4 no-print">
        ...
        <p className="text-neutral-700">Agenda - kyaliwajjala road Agenda, Kyaliwajjala - Naalya Rd, Kampala, Uganda • 0701 761271</p>
      </div>
    </div>
  );
};
